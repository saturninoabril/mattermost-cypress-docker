...
describe('autocomplete', () => {
    ...
    it('MM-T2199 @ autocomplete - username', () => {
        cy.visitAndWait(`/${testTeam.name}/channels/town-square`).wait(TIMEOUTS.FIVE_SEC);
            ...
            cy.apiLogin(testUser).then(() => {
                ...
                cy.apiLogout().then(() => {
                    cy.apiLogin(tempUser).then(() => {
                        cy.visitAndWait(`/${testTeam.name}`).wait(TIMEOUTS.FIVE_SEC);
    ...
    it('MM-T2209 @ autocomplete - not in DM, GM', () => {
        ...
        cy.apiCreateGroupChannel(userGroupIds).then(({channel: gmChannel}) => {
            ...
            cy.visitAndWait(`/${testTeam.name}/channels/${gmChannel.name}`).wait(TIMEOUTS.FIVE_SEC);
        ...
        cy.apiCreateDirectChannel([testUser.id, otherUser.id]).then(() => {
            ...
            cy.visitAndWait(`/${testTeam.name}/channels/${testUser.id}__${otherUser.id}`).wait(TIMEOUTS.FIVE_SEC);
